...
summon armor_stand ^ ^1.7 ^2.5 {Tags:["axe"],NoGravity:true,Marker:true,Invisible:true}
scoreboard players operation @s Com.player_ID = @e[tag=axe] Com.player_ID
scoreboard players add @e[tag=axe] swordage 1
execute store result score @e[tag=axe,scores={swordage=1}] Com.RotationX run scoreboard players get @s Com.RotationX
execute store result score @e[tag=axe,scores={swordage=1}] Com.RotationY run scoreboard players get @s Com.RotationY
